<?
    ...
    $img=CFile::ResizeImageGet($arResult['DETAIL_PICTURE']['ID'], 
        Array("width"=>980, "height"=>520), 
    ...
    {
        $arIBlockSection=CIBlockElement::GetList(
            Array("RAND" => "ASC"),
            Array("IBLOCK_ID" => 12),
            false,
            Array ("nTopCount" => 3)
        );
        while($ar = $arIBlockSection->GetNext())
        { 
            if(empty($ar["IBLOCK_SECTION_ID"]))
            {
                $ar['DETAIL_PAGE_URL']=str_replace("/products/","/products/0/",$ar['DETAIL_PAGE_URL']);
            }                          
            $arResult['PROPERTIES']['RELATED']['VALUE'][]=$ar;
        }
    }
    else
    {
        foreach($arResult['PROPERTIES']['RELATED']['VALUE'] as $id) $filter[]= $id;
        $arIBlockSection=CIBlockElement::GetList(
            Array(),
            Array('ID'=>$filter),
            false,
        ...
        unset($arResult['PROPERTIES']['RELATED']['VALUE']);
        while($ar = $arIBlockSection->GetNext())
        {   
            if(empty($ar["IBLOCK_SECTION_ID"]))
            {
            ...
            {
                $ar['DETAIL_PAGE_URL']=$ar['~DETAIL_PAGE_URL']; 
            }          
            $arResult['PROPERTIES']['RELATED']['VALUE'][]=$ar;
        }
    }
    if(empty($arResult['PROPERTIES']['RELATED_TITLE']['VALUE'])) $arResult['PROPERTIES']['RELATED_TITLE']['VALUE']="Другая продукция";
?>